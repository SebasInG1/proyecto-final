...
export const create = (inscribe: Inscribe, callback: Function) => {
    ...
    db.query(queryString, [inscribe.cod_e, inscribe.cod_a, inscribe.id_p, inscribe.grupo, inscribe.n1, inscribe.n2, inscribe.n3, inscribe.cod_i], (err) => {
        if (err) {
            callback(err);
        }
...
export const getById = (cod_i: number, callback: Function) => {
    const queryString = 'SELECT * FROM inscribe WHERE cod_i = ?';
    db.query(queryString, [cod_i], (err, result) => {
        if (err) {
            callback(err);
        }
...
export const update = (inscribe: Inscribe, callback: Function) => {
    ...
    db.query(
        queryString, 
        [inscribe.cod_e,inscribe.cod_a, inscribe.id_p, inscribe.grupo, inscribe.n1, inscribe.n2, inscribe.n3, inscribe.cod_e], (err) => {
        if (err) {
            callback(err);
        }
        callback(null, {
...
export const remove = (cod_i: number, callback: Function) => {
    const queryString = 'DELETE FROM inscribe WHERE cod_i = ?';
    db.query(queryString, [cod_i], (err) => {
        if (err) {
            callback(err);
        }
        callback(null, {
